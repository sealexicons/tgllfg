...
def register_rules(rules: list[Rule]) -> None:
    ...
    # Phase 11.B.2: 24 matrix-rule bindings here removed; binding
    # relocated to 4 NP-level wrap rules in ``cfg/nominal.py``
    # using 10.N inside-out designators. See
    # ``docs/fu-extension-audit.md`` §2.2 shipping outcome and the
    # ``Phase 11.B.2 sarili anaphora binding`` block in
    # ``cfg/nominal.py``. The 12 non-binding matrix transitive rules
    # above (NOM-GEN / GEN-NOM / NOM-GEN-DAT / GEN-NOM-DAT per
    # voice_spec) continue to handle the matrix-level GF
    # assignments unchanged; the binding now fires at the
    # reflexive NP's own f-structure via inside-out ANTECEDENT
    # binding (Dalrymple 2001 §14-15 canonical reflexive idiom).
