...
class TestPhase8aOutOfScope:
    """``pinagbintangan`` (mag-class ``pag-X-an`` LF PFV) is
    explicitly out of 8.A scope — the analyzer has no
    ``pag_an`` paradigm cell. Pin the current behavior so a
    future paradigm-extension PR flipping this to pass is a
    visible signal."""

    def test_pag_an_pf_still_unk(self) -> None:
        a = _first_analysis("pinagbintangan")
        # Either _UNK or no analysis — both indicate the form
        # isn't generated by any current paradigm cell.
